...
function addEntry(blog) {
  ...
  myPosts.appendChild(article_element);
  article_element.setAttribute("class", "post");

  let h3_element = document.createElement('h3');
  let myArticles = document.querySelectorAll("article");
  myArticles.forEach(article_element => article_element.appendChild(h3_element))
  h3_element.textContent = blog.title;
  h3_element.setAttribute("class", "blog-header");

  let p1_element = document.createElement('p');
  article_element.appendChild(p1_element);
  p1_element.setAttribute("class", "date");
  p1_element.textContent = blog.date.toLocaleDateString();

  let p2_element = document.createElement('p2');
  article_element.appendChild(p2_element);
  p2_element.textContent = blog.content;

  let btn = document.createElement('button');
  btn.textContent = "[x]";
  h3_element.appendChild(btn);
